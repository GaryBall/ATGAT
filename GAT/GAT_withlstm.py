...
class models:
    ...
    optimizer = 'adagrad', loss = 'mse', metrics=[]):
        ...
        print(K.int_shape(nbhd_vecs[0]))
        nbhd_vecs = [Concatenate(axis=-1)([nbhd_vecs[ts], graph_inputs[ts]]) for ts in range(lstm_seq_len)]

        nbhd_vec = Concatenate(axis=-1)(nbhd_vecs)
            # [Concatenate(axis=-1, name = "concatatc lstm{0} and input{0}".format(ts+1))(nbhd_vecs[ts], graph_inputs[ts]) for ts in range(lstm_seq_len)]
        print(K.int_shape(nbhd_vec))
        nbhd_vec = Reshape(target_shape = (lstm_seq_len, 12))(nbhd_vec)
        ...
        act_value = Activation('tanh')(lstm)
